...
import com.kingq.server.cache.KingQCache;
import com.kingq.server.config.Config;
import com.kingq.server.network.Connection;
import com.kingq.server.network.initialize.ClusterPublisherChannelInitializer;
import com.kingq.server.network.initialize.ServerChannelInitializer;
 ...
 */
public class KingQServer {

    private static final Logger LOGGER = KingQ.getLogger();
    ...
    public KingQServer(Config config) {
    ...
    public KingQServer(String host, int port, int backlog, boolean debug, int workerThreads, List<ClusterServer> cluster, int cleanupInterval, int snapshotInterval) {

        this.host = host;
        this.port = port;
        this.backlog = backlog;
        this.workerThreads = workerThreads;
        this.cache = new KingQCache(cleanupInterval, snapshotInterval);
    ...
    private void start() {
        ...
        LOGGER.log(Level.INFO, "KingQ server started on {0}:{1}", new Object[]{host, String.valueOf(port)});
    }
    ...
    public void clusterBroadcast(Connection connection, JSONObject data) {
        ...
        // if it comes from another KingQ server but also publish it
